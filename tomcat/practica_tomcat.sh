...
sudo docker rmi -f tomcat_imagen 2>/dev/null
...
echo -e "${VERDE}Arrancando el contenedor servidor-tomcat${NC}"
sudo docker run -d \
    --name servidor-tomcat \
    -p 8080:8080 \
    -p 8081:8081 \
    -p 8443:8443 \
    -p 8009:8009 \
...
curl -k -s -o /dev/null -w "HTTP status HTTPS: %{http_code}\n" https://localhost:8443/sitio1/hello
